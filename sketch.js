...
function draw() {
  ...
  for(var start=0;start <10;start++){
    fill(colorR1, colorG1, colorB1, random(oPace));
    noStroke();
    ellipse(random(width), random(height), random(60));
  }
  for(var start=0;start <10;start++){
    fill(colorR2, colorG2, colorB2, random(oPace));
    noStroke();
    ellipse(random(width), random(height), random(60));
  }
  
  for(var start=0;start <10;start++){
    fill(colorR3, colorG3, colorB3, random(oPace));
    noStroke();
    ellipse(random(width), random(height), random(60));
  }
...
function keyPressed(){
  ...
  for(var start=0;start <50;start++){
    fill(random(30),random(200),random(30), 255);
    noStroke();
    rect(random(width), random(height), random(70),random(70));
  }
